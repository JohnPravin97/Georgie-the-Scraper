...
def youtube_scraper(inp):
    ...
    driver = webdriver.Chrome(executable_path=r'C:\Users\johnp\Desktop\John - Projects\chromedriver.exe')
    ...
    for i,second_content in enumerate (first_content.find_all('div', class_='text-wrapper style-scope ytd-video-renderer')):
        try:
            ...
            channel_content = second_content.find('div', class_='style-scope ytd-channel-name')
            channel.append(channel_content.find('div', class_='hidden style-scope tp-yt-paper-tooltip').text.strip())  
...
def book_details(df, index):
    ...
    filepath = r"C:\Users\johnp\Desktop\John - Projects\Streamlit\Asus - Streamlit\Book Scraper Images\\" + book_name + ".png"
    try:
        try:
...
def spelling_checker(x):
    ...
    driver.close()
    try:
        first = soup.find('div', id='sp_requery').text
        crt_inp=' '.join(first.split()[3:]).split('.')[0]
    except:
        crt_inp=x
    return crt_inp
...
@st.cache(suppress_st_warning=False)
def multiply_celebs_checker(inp):
    ...
    if Description == "Topics referred to by the same term":
        first = soup.find('div', class_='mw-body-content mw-content-ltr').text.split('[edit]')
        for h in range (1, len(first)-1):
            results.append(first[h].split('\n')[1:-1])          
        for index, content in enumerate(soup.find_all('span', class_='mw-headline')):
            cnt.append(content.text)
    return results, cnt 
...
#### GitHub : <https://github.com/JohnPravin97> ''')
    st.sidebar.markdown('''<h3> <u> Special thanks to </u> </h3>''', unsafe_allow_html=True)
    st.sidebar.markdown('''
    ...
    ys = st.expander('Youtube Scraper', expanded=False)
    with ys:
    ...
    bs = st.expander('Book Scraper', expanded=False)
    with bs:
    ...
    cs = st.expander('Celebrity Scraper', expanded=False)
    with cs:
    ...
    if option == 'Book Scraper':
        ...
        if (not inp):
            ...
            if (st.button('Search') or state.flag ==True):
                ...
                st.write(spell_inp)
                spell_inp = '+'.join(spell_inp.split(' '))
                checker = '/'+ spell_inp
                books_save = pd.HDFStore(r'C:\Users\johnp\Desktop\John - Projects\Streamlit\Asus - Streamlit\bookscraper.h5')  
        ...
        if (not inp):
            ...
            if (st.button('Search') or state.flag ==True):
                st.write('Please wait - Georgie is scraping the information needed')
                df = youtube_scraper(inp)
                st.dataframe(df, width=10000, height=10000)
 
                index = st.number_input('Enter the index number to play the song:', min_value=0, max_value=5, step=1)
                state.flag = True
                if index != 0:
        ...
        if (not spell_inp):
            ...
            if (st.button('Search') or state.flag ==True):
                ...
                res, c = multiply_celebs_checker(inp)
                dic = {}
                if res !=[] and c !=[]:
                    '''#### We could find multiply celebrities by the given name'''
                    for ind, g in enumerate(res):
                        dic[c[ind]]=g
                    mul_picks= st.selectbox('Please Choose From the below', tuple(c))
                    opt = st.radio('Which Celebrity Details to Scrap', dic[mul_picks] )
                    k = opt.split(',')[0]
                    l = any(chr.isdigit() for chr in k)
                    if l:
                        inp = '_'.join((k.split(')')[0] + ')').split(' ')) 
                    else:
                        inp = '_'.join(k.split(' '))
                     
                checker = '/'+inp

                celebrity_save = pd.HDFStore(r'C:\Users\johnp\Desktop\John - Projects\Streamlit\Asus - Streamlit\celebrity.h5')               
                try:
                    ...
                    try:    
                        filepath = r"C:\Users\johnp\Desktop\John - Projects\Streamlit\Asus - Streamlit\Celebrity Images\\" + inp +".png"
                        ...
                        st.write('Image is not avaliable. please proceed with yes button to get details')
